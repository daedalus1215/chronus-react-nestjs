...
export const useTranscriptionWebSocket = ({
  noteId: _noteId, // Not needed for direct thoth connection
  onTranscription,
  enabled: _enabled, // Intentionally unused - connection is controlled manually
  ...
  useEffect(() => {
    const prevCallback = onTranscriptionRef.current?.toString().substring(0, 200);
    const prevIsEmpty = prevCallback?.includes('appendToDescription called but appendToDescriptionFn is not set yet') || 
                       prevCallback?.trim() === '() => {\n          }' ||
                       prevCallback?.trim() === '() => {}' ||
    ...
    const newCallback = onTranscriptionRef.current?.toString().substring(0, 200);
    const newIsEmpty = newCallback?.includes('appendToDescription called but appendToDescriptionFn is not set yet') ||
                       newCallback?.trim() === '() => {\n          }' ||
                       newCallback?.trim() === '() => {}' ||
    ...
    if (prevIsEmpty && !newIsEmpty) {
      ...
      console.warn('⚠️⚠️⚠️ Still empty callback in ref - editor may not have mounted yet');
    }
  }, [onTranscription]);
  ...
  const connect = useCallback(() => {
    ...
    if (wsRef.current) {
      console.log('Existing WebSocket found but not open, closing before creating new one');
      wsRef.current.close();
      wsRef.current = null;
    }
      ...
      try {
        ...
        if (
          ...
          if (typeof callback !== 'function') {
            console.error('❌ onTranscription callback is not a function:', typeof callback);
            return;
          }
          
          const callbackStr = callback.toString();
          // Check if it's the wrapper callback (should contain "onTranscriptionCallback" or "appendToDescriptionFn")
          const isWrapperCallback = callbackStr.includes('onTranscriptionCallback') || 
          ...
          try {
            callback(trimmed);
            console.log('✅ onTranscription callback executed - check if appendToDescription was called');
          } catch (err) {
  ...
  const sendAudioChunk = useCallback(
    (chunk: ArrayBuffer) => {
      ...
      if (wsRef.current?.readyState === WebSocket.OPEN && isRecordingRef.current && chunk && chunk.byteLength > 0) {
        try {
          // Log occasionally to avoid spam
          if (Math.random() < 0.01) {
            console.log(`Sending audio chunk to Thoth, size: ${chunk.byteLength} bytes`);
          }
