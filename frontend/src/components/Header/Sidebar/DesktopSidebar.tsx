...
export const DesktopSidebar: React.FC<DesktopSidebarProps> = () => {
  ...
  const { size: width, setSize, startResizing, handleKeyDown } = useResizablePane({
    localStorageKey,
    min: 0,
    max: 420,
    initial: 180,
          ...
          return (
            <ListItem key={item.path} disablePadding>
              <ListItemButton
                ...
                sx={{ justifyContent: width < 120 ? 'center' : 'flex-start', gap: width < 120 ? 0 : undefined }}
              >
                <ListItemIcon className={styles.navIcon} sx={{ minWidth: width < 120 ? 0 : undefined, justifyContent: 'center' }}>
                  <item.icon />
                </ListItemIcon>
                {width >= 120 && (
                  <ListItemText 
