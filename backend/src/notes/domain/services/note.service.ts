...
export class NoteService {
  ...
  async getNoteByIdWithCheckItems(noteId: number, userId: number): Promise<NoteWithCheckItems> {
    const note = await this.getNoteByIdTransactionScript.apply(noteId, userId);
    const checkItems = await this.checkItemsAggregator.findByNoteId(noteId, userId);
    return {
      note,
      checkItems,
    };
  ...
  async updateNoteWithCheckItems(
    ...
    const note = await this.updateNoteTransactionScript.apply(noteId, updateNoteDto, userId);
    const checkItems = await this.checkItemsAggregator.findByNoteId(noteId, userId);
    return {
      note,
      checkItems,
